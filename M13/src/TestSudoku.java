 ...
 *
 * @author Rui Huang Steven Ren
 */
public class TestSudoku {

    /**
     * This contains the calls to the various test methods. Uncomment the method calls and run to
     * execute the test cases.
     * <p>
     * These test cases are not intended to be comprehensive but provide some examples. Extend with
     * your own test cases to be comprehensive.
     *
     * @param args unused
     */
    ...
    private static void testReadBoardFromFile() {
        boolean error = false;
        {
            ...
            try {
                ...
                int[][] expected = {{9, 1, 0, 0, 0, 0, 4, 2, 7}, {0, 0, 0, 0, 0, 3, 9, 1, 5},
                    {2, 5, 4, 7, 0, 0, 6, 8, 0}, {4, 7, 0, 0, 8, 6, 0, 3, 2},
                    {0, 6, 0, 4, 0, 0, 0, 0, 8}, {5, 0, 0, 0, 1, 2, 0, 6, 0},
                    {3, 4, 0, 6, 2, 0, 0, 0, 1}, {0, 0, 0, 3, 0, 0, 0, 0, 0},
                    {0, 2, 6, 0, 0, 8, 0, 0, 9}};
...

        {
            ...
            try {
                ...
                error = true;
                System.out.println(
                    "testReadBoardFromFile 2: should have thrown exception for non-existing file.");
            } catch (FileNotFoundException e) {
    ...
    private static void testSaveBoardToFile() {
        boolean error = false;
        int[][] board =
            {{9, 1, 0, 0, 0, 0, 4, 2, 7}, {0, 0, 0, 0, 0, 3, 9, 1, 5}, {2, 5, 4, 7, 0, 0, 6, 8, 0},
                {4, 7, 0, 0, 8, 6, 0, 3, 2}, {0, 6, 0, 4, 0, 0, 0, 0, 8},
                {5, 0, 0, 0, 1, 2, 0, 6, 0}, {3, 4, 0, 6, 2, 0, 0, 0, 1},
                {0, 0, 0, 3, 0, 0, 0, 0, 0}, {0, 2, 6, 0, 0, 8, 0, 0, 9}};
        {
            try {
        ...
        {
            try {
                Sudoku.saveBoardToFile(board,
                    "nonExistingDirectory/testSaveBoardToFile_output.txt");
                error = true;
                System.out.println(
                    "testSaveBoardToFile 2: should have thrown exception for non-existing file.");
            } catch (FileNotFoundException e) {
                // expected, so not an error.
            }
        }
    ...
    private static void testCheckFullForBoard() {
...

        {
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testCheckFullForBoard 1: actual: " + actual + " expected: " + expected);
            }
        }

        {
            int[][] board = {{9, 1, 3, 3, 3, 3, 4, 2, 7}, {3, 3, 3, 3, 3, 3, 9, 1, 5},
                {2, 5, 4, 7, 3, 3, 6, 8, 3}, {4, 7, 3, 3, 8, 6, 3, 3, 2},
                {3, 6, 3, 4, 3, 3, 3, 3, 8}, {5, 3, 3, 3, 1, 2, 3, 6, 3},
                {3, 4, 3, 6, 2, 3, 3, 3, 1}, {3, 3, 3, 3, 3, 3, 3, 3, 3},
                {3, 2, 6, 3, 3, 8, 3, 3, 9}};
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testCheckFullForBoard 2: actual: " + actual + " expected: " + expected);
            }
        }
    ...
    private static void testFillNumber() {
...

        {
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testFillNumber 1: actual: " + actual + " expected: " + expected);
            }
...

        {
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testFillNumber 2: actual: " + actual + " expected: " + expected);
            }
            if (!Arrays.deepEquals(prevBoard, board)) {
                error = true;
...

        {
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testFillNumber 3: actual: " + actual + " expected: " + expected);
            }
            if (!Arrays.deepEquals(prevBoard, board)) {
                error = true;
...

        {
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testFillNumber 4: actual: " + actual + " expected: " + expected);
            }
            if (!Arrays.deepEquals(prevBoard, board)) {
                error = true;
...

        {
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testFillNumber 5: actual: " + actual + " expected: " + expected);
            }
            if (!Arrays.deepEquals(prevBoard, board)) {
                error = true;
...

        {
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testFillNumber 6: actual: " + actual + " expected: " + expected);
            }
    ...
    private static void testCheckConflictForBoard() {
...

        {
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testCheckConflictForBoard 1: actual: " + actual + " expected: " + expected);
            }
        }

        {
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testCheckConflictForBoard 2: actual: " + actual + " expected: " + expected);
            }
        }
    ...
    private static void testCheckConflictForArray() {
...

        {
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testCheckConflictForArray 1: actual: " + actual + " expected: " + expected);
            }
        }

        {
            ...
            if (expected != actual) {
                error = true;
                System.out.println(
                    "testCheckConflictForArray 2: actual: " + actual + " expected: " + expected);
            }
        }
    ...
    private static void testConstructLine() {
...

        {
            ...
            if (!expected.equals(actual)) {
                error = true;
                System.out.println(
                    "testConstructLine 1: actual: " + actual + " expected: " + expected);
            }
        }

        {
            ...
            if (!expected.equals(actual)) {
                error = true;
                System.out.println(
                    "testConstructLine 2: actual: " + actual + " expected: " + expected);
            }
        }
